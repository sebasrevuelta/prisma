...
1. We use the [Jest test framework](https://jestjs.io/). Its CLI is powerful and removes the need for npm scripts mostly. For most cases this is what you need to know:
   ...
   Note: Some test setups like functional tests have a different command to run the tests, [see below for details](#running-functional-tests).
...
You can provide the test path as a unnamed parameter to filter the tests being run: `pnpm run test:functional:code relationMode-17255-same-actions` or `pnpm run test:functional:code /logging/tests.ts`
